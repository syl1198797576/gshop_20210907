    ...
    <div class="ratings-content">
      <div class="overview">
        ...
        <div class="overview-right">
          ...
          <div class="delivery-wrapper"> <span class="title">送达时间</span> <span class="delivery">{{ info.deliveryTime }}
                                分钟</span> </div>
        </div>
      </div>
      <div class="split"></div>
      <div class="ratingselect">
        <div class="rating-type border-1px">
          <button class="block positive active" @click="setSelectType(2)">
            全部<span class="count">{{ ratings.length }}</span>
          </button>
          <button class="block positive" @click="setSelectType(0)">
            满意<span class="count">{{ positiveCount }}</span>
          </button>
          <button class="block negative" @click="setSelectType(1)">
            不满意<span class="count">{{ ratings.length-positiveCount }}</span>
          </button>
        </div>
        <div class="switch on"> <span class="iconfont icon-check_circle"></span> <span
  ...
  computed:{
    ...
    filterRatings () {
      ...
      return ratings.filter((rating) => {
        ...
        /*
        * 1. 如果selectType===2就不用管rateType
        *    否则需要让selectType===rateType
        * 2. 如果onlyShowText===false就不用管text的长度
        *    否则要让text.length>0
        * 3. 同时满足才返回结果
        * */
